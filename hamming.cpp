...
namespace Hamming_8_4
{
    ...
    uint8_t DecodeHelper(uint8_t x, int byteNum) {
        ...
        if (temp > 0) {
            ...
            case 15:
                d1 ^= 1;
                std::cout << "In " << byteNum << "th byte corrected error " << "in 7th bite." << std::endl;
                break;
            case 14:
                d2 ^= 1;
                std::cout << "In " << byteNum << "th byte corrected error " << "in 5th bite." << std::endl;
                break;
            case 13:
                d3 ^= 1;
                std::cout << "In " << byteNum << "th byte corrected error " << "in 3th bite." << std::endl;
                break;
            case 11:
                d4 ^= 1;
                std::cout << "In " << byteNum << "th byte corrected error " << "in 1st bite." << std::endl;
                break;
            default:
                std::cout << "There no errors in data bit." << std::endl;
            }
        }
    ...
    std::vector<uint8_t> Decode(const std::vector<uint8_t> bytes) {
        ...
        for (uint8_t byte : bytes) {
            ...
            if (res == 0b10000000) {
                std::cout << index << ". " << std::bitset<8>(byte) << " -> double error." << std::endl;
